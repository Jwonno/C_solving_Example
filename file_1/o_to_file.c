...
int main(){
    ...
    // "a.txt" 로 그냥 파일의 이름을 전달한다면 오직 '소스 파일과 동일한 경로에 들어있는 파일들' 을 찾게 된다.
    //만일 다른 폴더에 있는 a.txt 를 찾고 싶다면 그 경로를 넣어주면 된다.
    //예를 들어 C 드라이브의 사용자 폴더에 있는 a.txt를 원한다면 다음과 같이 하면 된다.
    // fp = fopen("C:\\users\\a.txt", "w");
